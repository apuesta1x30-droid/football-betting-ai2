...
# (DEBE IR ANTES DE IMPORTAR STATSTRACKER / ODDS_CLIENT)
# ==========================================
...
os.environ["THE_ODDS_API_KEY"] = st.secrets.get("THE_ODDS_API_KEY", os.getenv("THE_ODDS_API_KEY", ""))
os.environ["THE_ODDS_API_KEY_2"] = st.secrets.get("THE_ODDS_API_KEY_2", os.getenv("THE_ODDS_API_KEY_2", ""))
os.environ["THE_ODDS_API_KEY_3"] = st.secrets.get("THE_ODDS_API_KEY_3", os.getenv("THE_ODDS_API_KEY_3", ""))

from stats_tracker import StatsTracker
from odds_client import odds_get, get_keys
from auto_scan import compute_recalib
...
if _glosario:
    ...
    st.markdown("""
- **EV (%)** — Valor esperado: `(Prob. IA × cuota) − 1`. Es el margen que el sistema cree que tienes. ✅ Favorable: cuanto más alto, mejor (el umbral de notificación lo fija el auto-ajuste: 10-15% según calibración).
- **Prob. IA vs Prob. Casa** — La probabilidad que estima el modelo (ya corregida por la Capa B) frente a la que implica la cuota (`1/cuota`). Si la IA dice más que la casa, hay value.
...
""")
    ...
    st.markdown("""
Cada día a las **04:00 UTC**, el sistema mide su **gap de calibración** (EV declarado − yield real)
...
|---|---|---|---|---|
| > +25 pp | Sobreestimación muy fuerte | 15% | 1/8 | Máxima protección: solo pasa valor excepcional |
| +10 a +25 pp | Sobreestima fuerte | 12% | 1/8 | La IA promete más de lo que cumple: máxima exigencia y banca protegida |
...
# CONFIGURACIÓN
# ==========================================
# Las claves de The Odds API se leen vía odds_client (con tope mensual y rotación).
# El dashboard YA NO usa API-Football (política de riesgo cero con esa clave).
...
def load_recalib():
    """Capa B: MISMA función y mismos coeficientes que usa el pipeline (auto_scan)."""
    return compute_recalib(StatsTracker())


# ==========================================
# ESCANEO DE MERCADOS (con rotación de keys y tope mensual vía odds_client)
# ==========================================
@st.cache_data(ttl=1800, show_spinner="Escaneando mercados...")
def scan_all_markets():
    r = odds_get("sports/soccer/odds", {
        "regions": "eu,us",
        "markets": "h2h,totals",
        "oddsFormat": "decimal",
    }, tracker=StatsTracker())
    if r is None or r.status_code != 200:
        st.error("❌ Error en The Odds API (claves agotadas o tope mensual alcanzado).")
        return []
    return r.json()
...
def analyze_multi_market(models, fixtures_data, team_db, min_odd=1.8, max_odd=2.8,
    ...
    stats = {'total': 0, 'calculated': 0, 'today': 0}
    ...
    for event in fixtures_data:
        ...
        # Mercados inferidos: cuota calculada desde la probabilidad del modelo
        if min_odd <= (1 / dc_probs['1X']) <= max_odd:
            best_odds['DC_1X_CALC'] = 1 / dc_probs['1X']
            stats['calculated'] += 1
        if min_odd <= (1 / dc_probs['X2']) <= max_odd:
            best_odds['DC_X2_CALC'] = 1 / dc_probs['X2']
            stats['calculated'] += 1
        if min_odd <= (1 / dc_probs['12']) <= max_odd:
            best_odds['DC_12_CALC'] = 1 / dc_probs['12']
            stats['calculated'] += 1
        if min_odd <= (1 / prob_over05_ht) <= max_odd:
            best_odds['HT_Over_0.5_CALC'] = 1 / prob_over05_ht
            stats['calculated'] += 1
        if min_odd <= (1 / prob_btts) <= max_odd:
            best_odds['BTTS_Yes_CALC'] = 1 / prob_btts
            stats['calculated'] += 1
        if min_odd <= (1 / (1 - prob_btts)) <= max_odd:
        ...
        for market_key, odd in best_odds.items():
            ...
            # Capa B: MISMA corrección que aplica el pipeline antes de calcular EV
            if recalib:
                prob = max(0.03, min(0.97, recalib['alpha'] + recalib['beta'] * prob))

            ev = (prob * odd) - 1
            ev_percentage = ev * 100
            ev_ajustado = round(ev_percentage, 1)
...
st.markdown("Escaneando 6 mercados (4 reales + 2 inferidos) en 50+ ligas · **modo consulta** (no registra picks)")
...
st.sidebar.write(f"The Odds API Keys: {len(get_keys())}/3 (rotación y tope mensual activos)")
...
if 'df_bets' in st.session_state:
    ...
    with col_f3:
        selected_source = st.selectbox("🔖 Filtrar por Fuente:", options=["Todas", "The Odds API", "Cálculo (modelo)"])
    with col_f4:
    ...
    if not df_filtered.empty:
        ...
        if avg_odd < 1.8:
            st.markdown(f"🛡️ **Perfil de riesgo**: Conservador (cuota media {avg_odd:.2f})")
        elif avg_odd < 2.5:
...
if hist_stats['settled'] > 0:
    ...
    st.info("💡 Los picks se liquidan automáticamente con los cron de liquidación (03:30 y 12:00 UTC).")
else:
    st.info("📊 Aún no hay picks registrados. Los picks se guardarán a partir del próximo escaneo del bot.")
...
<p>🧠 Powered by XGBoost + The Odds API + fuentes gratuitas de resultados</p>
</div>
