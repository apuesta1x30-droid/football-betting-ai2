...
def load_models():
    models = {}
    model_files = {
        'over15': 'model_over15.pkl', 'over25': 'model_over25.pkl',
        'over35': 'model_over35.pkl', 'btts': 'model_btts.pkl',
        '1x2': 'model_1x2.pkl'
    }
...
def get_fixture_id(home_team, away_team, match_date):
    try:
        response = requests.get("https://v3.football.api-sports.io/fixtures", headers=API_FOOTBALL_HEADERS, params={"date": match_date[:10], "timezone": "UTC"}, timeout=10)
...
def get_api_football_odds(fixture_id):
    ...
    try:
        response = requests.get("https://v3.football.api-sports.io/odds", headers=API_FOOTBALL_HEADERS, params={"fixture": fixture_id}, timeout=10)
        odds_data = {}
...
def calculate_dc_probs(probs_1x2):
    return {'1X': probs_1x2[0] + probs_1x2[1], 'X2': probs_1x2[2] + probs_1x2[1], '12': probs_1x2[0] + probs_1x2[2]}
...
def scan_value_bets():
    ...
    if not THE_ODDS_API_KEY:
        logger.error("❌ THE_ODDS_API_KEY no configurada"); return
    ...
    params = {"regions": "eu,us", "markets": "h2h,totals", "oddsFormat": "decimal", "apiKey": THE_ODDS_API_KEY}
    
    try:
        response = requests.get(url, params=params, timeout=15)
        if response.status_code != 200:
            logger.error(f"❌ Error API: {response.text}"); return
        fixtures = response.json()
        logger.info(f"📡 {len(fixtures)} partidos obtenidos")
    except Exception as e:
        logger.error(f"❌ Error API: {e}"); return
    ...
    for event in fixtures:
        ...
        match_time = datetime.fromisoformat(commence_time.replace('Z', '+00:00')) if 'Z' in commence_time else datetime.fromisoformat(commence_time)
        
        if match_time.date() != now.date(): continue
        stats['total'] += 1
        
        league, home_team, away_team = event.get("sport_title", "Unknown"), event["home_team"], event["away_team"]
        home_stats, away_stats = get_team_stats(home_team, team_db), get_team_stats(away_team, team_db)
        
        features = pd.DataFrame([{
            'Home_Form_Pts': home_stats['Last_Form_Pts'], 'Away_Form_Pts': away_stats['Last_Form_Pts'],
            'Form_Diff': home_stats['Last_Form_Pts'] - away_stats['Last_Form_Pts'],
            'Home_Goals_Scored': home_stats['Last_Goals_Scored_Avg'], 'Away_Goals_Conceded': away_stats['Last_Goals_Conceded_Avg'],
            'Goal_Threat_Diff': home_stats['Last_Goals_Scored_Avg'] - away_stats['Last_Goals_Conceded_Avg'],
            'Combined_Over25_Rate': (home_stats['Last_Over25_Rate'] + away_stats['Last_Over25_Rate']) / 2,
            'Combined_BTTS_Rate': (home_stats['Last_BTTS_Rate'] + away_stats['Last_BTTS_Rate']) / 2
        }])
        ...
        for bookmaker in event.get("bookmakers", []):
            for market in bookmaker.get("markets", []):
                ...
                for outcome in market.get("outcomes", []):
                    odd = outcome["price"]
                    if odd < MIN_ODD or odd > MAX_ODD: continue
                    key = None
                    if mk == "h2h":
                        if outcome["name"] == home_team: key = f"1X2_{home_team}"
                        elif outcome["name"] == "Draw": key = "1X2_Draw"
                        elif outcome["name"] == away_team: key = f"1X2_{away_team}"
        ...
        for dc_key, prob_key in [('1X', '1X'), ('X2', 'X2'), ('12', '12')]:
            ...
            if api_val and MIN_ODD <= api_val <= MAX_ODD:
                best_odds[f'DC_{dc_key}'] = api_val; stats['api_football'] += 1
            elif MIN_ODD <= calc_val <= MAX_ODD:
                best_odds[f'DC_{dc_key}_CALC'] = calc_val; stats['calculated'] += 1
                
        ht_api = next((v for k, v in api_odds.items() if '6_Over' in k and '0.5' in k), None)
        ht_calc = 1 / prob_ht
        if ht_api and MIN_ODD <= ht_api <= MAX_ODD:
            best_odds['HT_Over_0.5'] = ht_api; stats['api_football'] += 1
        elif MIN_ODD <= ht_calc <= MAX_ODD:
            best_odds['HT_Over_0.5_CALC'] = ht_calc; stats['calculated'] += 1
        ...
        for mk, odd in best_odds.items():
            ...
            if mk.startswith("1X2_"):
                tp = mk.split("_")[1]
                if tp == home_team: prob, name = probs_1x2[0], f"1X2 - {home_team}"
                elif tp == "Draw": prob, name = probs_1x2[1], "1X2 - Empate"
                elif tp == away_team: prob, name = probs_1x2[2], f"1X2 - {away_team}"
            elif mk.startswith("Over_"):
                p = float(mk.split("_")[1])
                if p == 1.5: prob, name = models['over15'].predict_proba(features)[0][1], "Over 1.5 Goles"
                elif p == 2.5: prob, name = models['over25'].predict_proba(features)[0][1], "Over 2.5 Goles"
                elif p == 3.5: prob, name = models['over35'].predict_proba(features)[0][1], "Over 3.5 Goles"
            ...
            if prob:
                ...
                if ev * 100 > EV_THRESHOLD_MIN:
                    value_bets.append({"Liga": league, "Partido": f"{home_team} vs {away_team}", "Hora": commence_time[:16].replace('T', ' '), "Mercado": name, "Cuota": odd, "Prob. IA": prob, "Prob. Casa": 1/odd, "EV (%)": ev * 100, "Fuente": "Cálculo" if is_calc else "API-Football"})
